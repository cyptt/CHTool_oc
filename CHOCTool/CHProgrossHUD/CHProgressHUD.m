...
#define  CHMainThreadAssert() NSAssert([NSThread isMainThread], @" CHProgressHUD needs to be accessed on the main thread.");

CGFloat const  CHProgressMaxOffset = 1000000.f;

static const CGFloat  CHDefaultPadding = 4.f;
static const CGFloat  CHDefaultLabelFontSize = 16.f;
static const CGFloat  CHDefaultDetailsLabelFontSize = 12.f;
...
@interface  CHProgressHUDRoundedButton : UIButton
@end
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
     CHProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ ( CHProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
             CHProgressHUD *hud = ( CHProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType =  CHProgressHUDAnimationFade;
    _mode =  CHProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:( CHProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type ==  CHProgressHUDAnimationZoom) {
        type = animatingIn ?  CHProgressHUDAnimationZoomIn :  CHProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type ==  CHProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type ==  CHProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type ==  CHProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type ==  CHProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    if (self.hasFinished) {
     ...
     CHProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id< CHProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    UIColor *defaultColor = self.contentColor;
     ...
     CHBackgroundView *backgroundView = [[ CHBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style =  CHProgressHUDBackgroundStyleSolidColor;
    ...
    _backgroundView = backgroundView;
     ...
     CHBackgroundView *bezelView = [ CHBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize: CHDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize: CHDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [ CHProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize: CHDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[ CHRoundProgressView class]];
     ...
     CHProgressHUDMode mode = self.mode;
    if (mode ==  CHProgressHUDModeIndeterminate) {
        ...
        [indicator removeFromSuperview];
        indicator = [[ CHBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
        ...
        if (!isRoundIndicator) {
            ...
            [indicator removeFromSuperview];
            indicator = [[ CHRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
    ...
    else if (mode ==  CHProgressHUDModeCustomView && self.customView != indicator) {
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[ CHProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[ CHProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[ CHRoundProgressView class]]) {
         CHRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [ CHRoundProgressView appearanceWhenContainedIn:[ CHProgressHUD class], nil];
#else
        appearance = [ CHRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[ CHProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            (( CHRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            (( CHRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[ CHBarProgressView class]]) {
         CHBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [ CHBarProgressView appearanceWhenContainedIn:[ CHProgressHUD class], nil];
#else
        appearance = [ CHBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[ CHProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            (( CHBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            (( CHBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
     CHBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ?  CHDefaultPadding : 0.f;
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode ==  CHProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation  CHBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style =  CHProgressHUDBackgroundStyleBlur;
...
- (void)setStyle:( CHProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    self.effectView = nil;
     ...
     CHProgressHUDBackgroundStyle style = self.style;
    if (style ==  CHProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style ==  CHProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation  CHProgressHUDRoundedButton
