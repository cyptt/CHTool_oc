...
//
//  CHKVStorage.h
//  CHCache <https://github.com/ibireme/CHCache>
//
...
/**
 CHKVStorageItem is used by `CHKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface CHKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `CHKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache),
   use CHKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use CHKVStorageTypeFile to get better performance.
 * You can use CHKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, CHKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    CHKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    CHKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    CHKVStorageTypeMixed = 2,
};



/**
 CHKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for CHKVStorage is `initWithPath:type:`.
 ...
 */
@interface CHKVStorage : NSObject
...
@property (nonatomic, readonly) CHKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(CHKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is CHKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is CHKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is CHKVStorageTypeMixed, then the item.value will be saved to file
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(CHKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 CHKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is CHKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is CHKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is CHKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable CHKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable CHKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `CHKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<CHKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `CHKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<CHKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
