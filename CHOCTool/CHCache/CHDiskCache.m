...
static int64_t _CHDiskSpaceFree() {
    NSError *error = nil;
...
/// String's md5 hash.
static NSString *_CHNSStringMD5(NSString *string) {
...
static void _CHDiskCacheInitGlobal() {
...
static CHDiskCache *_CHDiskCacheGetGlobal(NSString *path) {
    if (path.length == 0) return nil;
    _CHDiskCacheInitGlobal();
...
static void _CHDiskCacheSetGlobal(CHDiskCache *cache) {
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _CHDiskSpaceFree();
...
- (NSString *)_filenameForKey:(NSString *)key {
    ...
    if (!filename) filename = _CHNSStringMD5(key);
    return filename;
}
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"CHDiskCache init error" reason:@"CHDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    CHDiskCache *globalCache = _CHDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    CHKVStorageType type;
    if (threshold == 0) {
        type = CHKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = CHKVStorageTypeSQLite;
    } else {
        type = CHKVStorageTypeMixed;
    }
    
    CHKVStorage *kv = [[CHKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    CHKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [CHDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [CHDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != CHKVStorageTypeSQLite) {
