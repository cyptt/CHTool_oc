...
static UIApplication *_CHSharedApplication() {
...
@implementation CHKVStorageItem
@end

@implementation CHKVStorage {
...
- (CHKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    CHKVStorageItem *item = [CHKVStorageItem new];
...
- (CHKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    CHKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            CHKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"CHKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:@"" type:CHKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(CHKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"CHKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > CHKVStorageTypeMixed) {
        NSLog(@"CHKVStorage init error: invalid type: %lu.", (unsigned long)type);
        return nil;
    }
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtPath:path
        ...
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kTrashDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error]) {
        NSLog(@"CHKVStorage init error:%@", error);
        return nil;
    }
    ...
    if (![self _dbOpen] || ![self _dbInitialize]) {
        ...
        if (![self _dbOpen] || ![self _dbInitialize]) {
            [self _dbClose];
            NSLog(@"CHKVStorage init error: fail to open sqlite db.");
            return nil;
        }
    }
...
- (void)dealloc {
    UIBackgroundTaskIdentifier taskID = [_CHSharedApplication() beginBackgroundTaskWithExpirationHandler:^{}];
    [self _dbClose];
    if (taskID != UIBackgroundTaskInvalid) {
        [_CHSharedApplication() endBackgroundTask:taskID];
    }
}
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == CHKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != CHKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case CHKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case CHKVStorageTypeFile:
        case CHKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (CHKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (CHKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    CHKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (CHKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    CHKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
